/* frameless @ 0x20568: search a 10-byte-record array until rec[8] == param_3, tracking
   ...
   point (g_10b22, g_10b24) lies strictly inside the record's box (rec[0]/[4] x, rec[2]/[6]
   y), take rec[8]. Return the byte result.

   PARKED near-miss (NOT matched; logic correct). Loop-invariant-hoisting + register wall:
   -oneatx hoists the globals g_10b22/g_10b24 out of the box-test loop into callee-saved
...
unsigned char FUN_00020568(unsigned char *recs, unsigned char param_2, unsigned char param_3)
{
    ...
    if (param_2 == 0) {
        ...
        while (recs[8] != param_3) {
            if (*(unsigned short *)recs < g_10b22 &&
                g_10b22 < *(unsigned short *)(recs + 4) &&
                *(unsigned short *)(recs + 2) < g_10b24 &&
                g_10b24 < *(unsigned short *)(recs + 6))
                result = recs[8];
            recs += 0xa;
        }
    }

...
void menu_list_draw_b(void)
{
    ...
    for (i = 0;; ++i) {
        ...
        if (*(unsigned short *)(g_7bf4 + wi * 0x1f5 + 0x11) == 0x960) {
            draw_ui_text(*(char **)((char *)g_4b10 + g_language * 4 + g_list_recs[wi * 0x1eb] * 12),
                         0x1f8, y, 0xe, 0xdd, g_text_pal, -2, 6, 0, 0);
            y += 0xc;
        }
    }
    if ((r = menu_list_hit_test(0x14, g_mouse_x, g_mouse_y, &sel, 0, 0xfa, 0x35, 0x139, 0xab, 0x38)) != 0) {
        draw_ui_text(g_4b10_2[g_list_recs[(r - 1) * 0x1eb]][g_language],

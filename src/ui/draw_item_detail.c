...
extern void draw_ui_text(char *s, unsigned short x, int a3, int a4, unsigned short a5,
                         ...
                         int a10);
extern char draw_wrapped_text(char *s, int x0, int y0, int w, int h,
                         unsigned char *font, unsigned short a7, unsigned char a8,
                         signed char a9, int a10, char **out);

void draw_item_detail(unsigned char param1, unsigned char param2)
{
    char buf[40];
    unsigned short y;
    ...
    if (param1 == 2) {
        if (param2 == 0x11)
            draw_sprite_rle_buf(0x1f6, 0x6a, g_text_pal + 0x1d4);
        else
            draw_sprite_rle_buf(0x1f6, 0x6a, g_text_pal + (param2 + 0x3f) * 6);

        sprintf(buf, g_19c,
                     *(char **)((char *)g_4b10 + g_language * 4
                                + g_7bf4[(param2 - 1) * 0x1f5] * 12));
        draw_ui_text(buf, 0x1f8, 0xc4, 0xe, 0x54, g_text_pal, -2, 6, 0, 0);

        draw_ui_text(*(char **)((char *)g_47a4 + g_language * 4), 0x1f8, 0xd0, 0xf, 0x54, g_text_pal, -2, 8, 0, 0);
        sprintf(buf, g_184, *(int *)(g_7bf4 + (param2 - 1) * 0x1f5 + 1));
        draw_ui_text(buf, 0x22c, 0xd0, 0xe, 0x54, g_text_pal, -2, 6, 0, 0);

        y = 0xdc;
        if (g_frame_enable[param2] != 0) {
            draw_ui_text(*(char **)((char *)g_47b0 + g_language * 4), 0x1f8, 0xdc, 0xf, 0x54, g_text_pal, -2, 8, 0, 0);
            sprintf(buf, g_184, *(short *)(g_7bf4 + (param2 - 1) * 0x1f5 + 5) + 1);
            draw_ui_text(buf, 0x22c, 0xdc, 0xe, 0x54, g_text_pal, -2, 6, 0, 0);
            y = 0xe8;
        }

        draw_ui_text(*(char **)((char *)g_47bc + g_language * 4), 0x1f8, y, 0xf, 0x54, g_text_pal, -2, 8, 0, 0);
        sprintf(buf, g_184, *(short *)(g_7bf4 + (param2 - 1) * 0x1f5 + 0xb));
        draw_ui_text(buf, 0x22c, y, 0xe, 0x54, g_text_pal, -2, 6, 0, 0);
        y += 0xc;

        if (g_frame_enable[param2] != 0) {
            draw_ui_text(*(char **)((char *)g_47c8 + g_language * 4), 0x1f8, y, 0xf, 0x54, g_text_pal, -2, 8, 0, 0);
            sprintf(buf, g_184, *(int *)(g_7bf4 + (param2 - 1) * 0x1f5 + 7));
            draw_ui_text(buf, 0x22c, y, 0xe, 0x54, g_text_pal, -2, 6, 0, 0);
        }
    } else {
        draw_ui_text(*(char **)((char *)g_4a38 + g_language * 4
                                 + g_list_recs[(param2 - 1) * 0x1eb] * 12),
                     0x1f8, 0x6e, 0xe, 0x54, g_text_pal, -2, 6, 0, 0);

        draw_ui_text(*(char **)((char *)g_47a4 + g_language * 4), 0x1f8, 0x7e, 0xf, 0x54, g_text_pal, -2, 8, 0, 0);
        sprintf(buf, g_184, *(int *)(g_list_recs + (param2 - 1) * 0x1eb + 1));
        draw_ui_text(buf, 0x22c, 0x7e, 0xe, 0x54, g_text_pal, -2, 6, 0, 0);

        if ((char *)g_4960 + g_list_recs[(param2 - 1) * 0x1eb + 5] * 12 != 0) {
            draw_wrapped_text(*(char **)((char *)g_4960

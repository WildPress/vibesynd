/* frameless @ 0x2e4f8: search up to 4 directions for a valid projectile step.
   local_10 = obj[0x1a] + param_2; each iteration test path_probe_0x40 at the position
...
extern int path_probe_0x40(int x, int y, int z, unsigned char *obj);
void find_projectile_step(unsigned char *obj, unsigned char param_2)
{
    unsigned char local_10;
    short i;
    local_10 = obj[0x1a] + param_2;
    for (i = 0; i < 4; i++) {
        if ((short)path_probe_0x40(

 ...
 * size via FUN_000179f8; if flags&4, malloc that size into 'out' (fail->close+0)
...
extern int FUN_000179f8(char *name, int flags);
...
#pragma aux FUN_00017b48 modify [eax ecx edx ebx];
...
void *FUN_00017b48(char *name, int flags, void *out)
{
    ...
    FUN_00017998(fd, tbl, flags, hdr, 0xac);
    ...
    if (k1 < *(unsigned int *)(hdr + 0x44)) {
        do {
            pos2c = FUN_00017998(fd, pos, flags, rec, 0x18);
            uv = 0;
            if (uv < *(unsigned int *)(rec + 0x10)) {
                pos = tbl + *(unsigned int *)(hdr + 0x48);
                do {
                    posInner = FUN_00017998(fd, pos, flags, in8, 8);
                    if (uv == 0) {
    ...
    pos = FUN_00017998(fd, tbl + *(unsigned int *)(hdr + 0x68), flags, &prev, 4);
    r34 = 0;
    if (r34 < *(unsigned int *)(hdr + 0x14)) {
        r28 = 0;
        do {
            pos = FUN_00017998(fd, pos, flags, &cur, 4);
            if (prev != cur) {
                savedIdx = r28;
                uv = prev + tbl + *(unsigned int *)(hdr + 0x6c);
                pos30 = pos;
                do {
                    pos = FUN_00017998(fd, uv, flags, b18, 1);
                    pos = FUN_00017998(fd, pos, flags, b10, 1);
                    if ((b18[0] & 7) == 0) {
                        if ((flags & 1) == 0)
                            FUN_0003a89d(fd);
                        return (void *)0;
                    ...
                    pos = FUN_00017998(fd, pos, flags, b1c, 2);
                    pos = FUN_00017998(fd, pos, flags, b14, 1);
                    uv = FUN_00017998(fd, pos, flags, b20, 2);

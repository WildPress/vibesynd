/* frameless @ 0x2ad58 -- mission targeting-cursor / action RESOLUTION dispatcher.
   ...
   Direction tables g_dir_dx/g_dir_dy[256]. Screen/scroll g_mouse_x/g_mouse_y/g_52f8.
   g_5114[] = 4 mission-map HUD regions (18-byte stride, .x/.y at +0/+2).

   STRUCTURE (top to bottom):
     * early guards: g_e285/radar -> 0x2; g_e2a4 -> 0x10; g_e28x -> set g_e124;
       g_e297&g_e296&g_e2a3 -> 0x17.
     * fast path (already have a target ped selected in g_10b14): -> 0x1a, or set
       g_e124 to the ped index if g_e120==1.
     ...
     * adjacency / line-of-sight pass over the block (g_e120==2) or the selected ped
       -> 0x16.
     * clamp cursor to world/scroll, set the reticle window (g_10b1c/1e/20), g_10b39.
     * draw_targeting_reticle(rec).
     * FINAL DISPATCH  switch (g_e120)  (orig uses a 5-entry jump table @ cs:0x1d5fc):
         case 0            -> return.
         case 1 / case 2   -> movement/aim commit CHAIN (0x2b44c..): a sequence of
                              guarded emitters, each returns or falls to the next:
                                ...
                                g_10b39&g_10b3f        -> 0x3
                                g_10b3f&g_10b22>=0x80  -> commit pending target:
                                    g_10b12 -> 0x19 or 0x9 ; g_10b1a -> 0x5 ;
                                    g_10b14 -> 0x18/0x4 ; else -> 0x3
...
after_scan:                                          /* 0x2b01b */
    ...
    /* ---- final mode dispatch: switch (g_e120) (jump table @ cs:0x1d5fc) ---- */
    switch (g_e120) {
    case 0:
    ...
    case 2:
        ...
        if (g_10b39 && g_10b3f) {                         /* 0x2b4aa -> action 0x3 */
            *((unsigned char *)p + 0xd) = 0x3;
            p[0] = g_10b20; p[1] = g_10b1e; p[2] = g_10b1c;
            g_10b3f = 0; g_sel_cursor = 0;
            r = 0; return r;
        }
        if (g_10b3f && g_10b22 >= 0x80) {                 /* 0x2b4fd -- commit pending target */
            ...
            if (g_10b14 != 0) {                           /* 0x2b5a1 -> action 0x18/0x4 */
        ...
        if (g_10b39 && g_10b3e) {                         /* 0x2b62f -- clamp reticle */
            clamp_point_box((g_10b20 - 0xa00 - (int)g_10b1c) >> 7,
                            (g_10b1e - 0x200 - (int)g_10b1c) >> 7);
            g_10b3e = 0; g_e114 = 0;
            r = 0; return r;
        }

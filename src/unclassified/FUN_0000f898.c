 ...
 * passability flag via a triple indirection g_a510[g_10ac0[ g_map_cols[row+col][z] ]]. Checks 4 corners
...
int FUN_0000f898(short param_1, short param_2, short param_3)
{
    if (param_3 < 0x600) {
        ...
        int z = param_3 / 0x80;
        int row2;
        if (g_a510[g_10ac0[g_map_cols[row + col][z]]] != 0)
            return 1;
        row2 = ((param_2 + 0x10) % 0x6000) / 0x100 * 0x80;
        if (g_a510[g_10ac0[g_map_cols[row2 + col][z]]] != 0)
            return 1;
        col = (((int)param_1 + 0x10) & 0xff00) / 0x100;
        if (g_a510[g_10ac0[g_map_cols[row + col][z]]] != 0)
            return 1;
        if (g_a510[g_10ac0[g_map_cols[col + row2][z]]] != 0)
            return 1;
    }
    return 0;
}

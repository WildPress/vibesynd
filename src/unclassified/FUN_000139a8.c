 ...
 * rule bit in b[1]: &4 -> b[1]&1; &2 -> owner p = ((n - g_pool_a)/0x5c)/8, if
 * player-rec flag (g_player_flags[p*0x417] & 2) then p==idx forces 0 else b[1]&4,
...
char FUN_000139a8(unsigned char *b, unsigned char *n, unsigned char idx)
{
    ...
    if (n[0x1c] & 4) {
        ...
        p = (int)((n - g_pool_a) / 0x5c) / 8;
        if (g_player_flags[p * 0x417] & 2) {
            if (p == idx)
                result = 0;
            else if (b[1] & 4)
                result = 1;

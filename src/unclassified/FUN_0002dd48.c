/* FUN_0002dd48 @ 0x2dd48 - entity event/flag-word (+0xc) dispatcher.
 ...
 * bit spawns a sub-object via FUN_0002d9e8(g_entity_pool+node[0x16], p); on success it
...
extern unsigned short FUN_0002d9e8(unsigned char *node, unsigned char *entity);
extern void FUN_0000daa8(int a, unsigned char *entity);
unsigned short FUN_0002dd48(unsigned char *p)
{
    ...
    if (f != 0) {
        *(unsigned short *)(p + 0xc) = 0;
        ...
        if (f & 0x400) {
            if (FUN_0002d9e8(g_entity_pool + *(unsigned short *)(p + 0x16), p) != 0) {
                p[0x58] = 0x1e;
                p[0xa] |= 8;

 ...
 *          g_5114[i*0x12 + 8], and draw only the inventory icon (skip the bars).
 ...
 *      4px progress bar for each enabled type (g_a69a[frame]!=0), value =
...
extern unsigned char  g_5258[];     /* aux-bar panel table, stride 0x12: x@+0,y@+2 */
extern unsigned short g_a73a[];     /* per-frame max/quantity table */
extern unsigned char  g_a69a[];     /* per-frame enable flags */
...
#define HUD_X(i) (*(unsigned short *)(g_5114 + (i) * 0x12))
#define HUD_Y(i) (*(unsigned short *)(g_5114 + (i) * 0x12 + 2))
    ...
    /* second pass: auxiliary progress bars hung off the reference entity */
    {
        ...
        while (id != 0) {
            ...
            if (g_a69a[frame] != 0) {
                ...
                if (prev != val) {
                    if (prev > 0)
                        FUN_0003feb3(*(unsigned short *)(g_5258 + i * 0x12) + 4,
                                     *(unsigned short *)(g_5258 + i * 0x12 + 2) + 0x18,
                                     prev, 4, 8);
                    if (val > 0)
                        FUN_0003feb3(*(unsigned short *)(g_5258 + i * 0x12) + 4,
                                     *(unsigned short *)(g_5258 + i * 0x12 + 2) + 0x18,

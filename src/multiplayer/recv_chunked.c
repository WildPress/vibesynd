/* Proposed name: recv_chunked -- the receive counterpart of xfer_chunked (0x28878,
   which is void/send). Receives param_4 bytes in whole chunks of g_net_chunk_size, verifying
   each chunk: it calls netbios_recv95(p1,p2,p3,0) for param_4/g_net_chunk_size whole chunks
   (g_net_chunk_size is the 16-bit global chunk size, re-read each iter; callee returns short).
   If a chunk's returned count != g_net_chunk_size it stops and returns that count (short read).
   Else it advances the buffer p3 by g_net_chunk_size. After the whole chunks it does one final
   (remainder) call: if that count == param_4 % g_net_chunk_size it returns param_4 (all bytes
   ...
   edx,ax`). It also coaxes out the target's g_net_chunk_size-reconstruct idiom in the loop
   ...
   call result in EAX (CWDE in place) and loads g_net_chunk_size into CX (`66 8b 0d`, 7 bytes);
   ours evicts the result to ECX so AX is free for the 1-byte-shorter `66 a1` moffs
   load of g_net_chunk_size -- a genuine cost-model coin flip in the loop BODY (the loop
   CONDITION already forces g_net_chunk_size into CX for the DIV, and matches byte-for-byte).

/* frameless @ 0x272b8: multiplayer player-record sync barrier. Logs g_368c; only
   ...
   g_player_recs[i] to every other active player j (g_peer_active[j] != 0, j != self) via 0x28878
   on channel g_net_channels[j]/{dword,word}, returning -1 on any send error; otherwise if
   slot i is active (g_peer_active[i] == 1), busy-RECEIVE player i's record into g_player_recs[i]
...
extern unsigned char g_peer_active[];
...
extern struct chan g_net_channels[];
extern char g_368c[];
extern char g_36a0[];
extern char g_36ac[];
  ...
  if ((g_num_players > 1) && (g_cur_player == ((short) param_1)))
  {
    for (i = 0; i < g_num_players; i++)
    {
      if (i == g_cur_player)
      {
        ...
        {
          if ((g_peer_active[j] != 0) && (j != g_cur_player))
          {
            printf(g_36a0);
            if (0 != xfer_chunked(g_net_channels[j].a, g_net_channels[j].b, g_player_recs[i], 0x417))
              return -1;
          }
        }
        ...
        if (1 == g_peer_active[i])
      {
        printf(g_36ac);
        while (recv_chunked(g_net_channels[i].a, g_net_channels[i].b, g_player_recs[i], 0x417) != 0x417)
        {
        }

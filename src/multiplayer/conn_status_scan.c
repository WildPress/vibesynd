/* 0x279f8 -- scan the far-pointer slot table at 0x10644 (6-byte far ptrs,
 * lgs stride *6). For each slot i < g_num_players: if p[2] and p[0x31]!=0xff, and
 * the slot isn't already marked in g_peer_active and isn't self (g_cur_player), report
 * p[0x31] if nonzero via 0x289a8(str 0x376c, 0x10a, c), mark 1 and count;
 * dead slots mark 0. Single shared store g_peer_active[i]=v (if/else, success path
 * falls through; continue guards skip the store). i unsigned short.
 * Recipe: -4s -oneatx -zp8 -s -zq
 */
extern char __far *g_net_channels[];
extern unsigned char g_peer_active[];
extern short g_cur_player;
extern short g_num_players;
extern char g_376c[];
extern void report_net_status(char *s, int b, int c);

int conn_status_scan(void)
{
    int n = 0;
    unsigned short i;

    for (i = 0; i < g_num_players; i++) {
        char __far *p = g_net_channels[i];

        if (p[2] != 0 && p[0x31] != 0xff) {
            if (g_peer_active[i] != 0)
                continue;
            if (i == g_cur_player)
                continue;
            if (p[0x31] != 0)
                report_net_status(g_376c, 0x10a, (unsigned char)p[0x31]);
            g_peer_active[i] = 1;
            n++;
        } else {
            g_peer_active[i] = 0;
        }
    }
    return n;
}

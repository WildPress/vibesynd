/* PARKED NEAR-MISS (ours 314B vs target 313B, raw LCS 247/313, structure
   ...
   same `if (FUN_00027d88(...) == -1)` spelling that emits movsx ecx,ax
   here -- allocator state, not source-reachable. Fuzzer/cpermute may close.
   Recipe: -4s -oneatx -zp8 -s -zq

   FUN_00028228 @ 0x28228: NetBIOS session-op (opcode 0x91), same family as
   ...
   (db-transcribed pragma), pads with g_377c (far strcat) until far strlen
...
int FUN_00028228(unsigned int off, unsigned short sel, char *name,
                 unsigned short async, char rto, char sto)
{
    ...
    while (fstrlen91(q) < 0xf)
        fstrcat91(q, (unsigned char __far *)g_377c);

/* unload_all_drivers -- SKIP-ASM-IDIOM (per-voice service loop, 111B).
 * disasm: framed; pushfd; cli;
 *   for (i=0; i<0x10; i++) if (g_bcfa[i]) {
 *       if (g_bd3a[i] != -1) free_seq_slot(g_bd3a[i]);
 *       stop_voice(i, [ebp+8]);
 *   }
 *   stop_all_seqs();
...
#pragma aux unload_all_drivers modify [eax ebx ecx edx esi edi ebp];
void unload_all_drivers(void) { __b000395b61(); __b000395b62(); }

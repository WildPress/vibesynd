; start_seq @ 00039747  (48 bytes) -- hand-written assembly (fully commented).
;
; start_seq: start/resume a sequence. If the slot's state word is 1 (loaded/paused),
; bump it to 2 (running); otherwise leave it untouched. The 1->2 transition is what the
; timer ISR keys on to begin servicing the slot. Paired with pause_seq (2->1, pause).
;

; start_voice @ 0x399bd  (197 bytes) -- hand-written assembly (fully commented).
;
; start_voice: start playback on a voice. Given a voice index and four playback
...
;   0xbd3a  g_voice_handle[16]   per-voice handle table (set here from alloc_seq_slot)
;   0xbd7a  g_voice_active[16]   per-voice active flag (set to 1 at the end)
; Calls: voice_get_driver_obj (cmd 0x64 -> driver object), driver_msg_lookup (resolve cmd 0x67
;   handler), alloc_seq_slot (build the voice handle), timer_rate_critsec (set callback
;   rate), sound_dispatch_trampoline (cmd 0x66 = play), start_seq (post-start hook).
;
...
start_voice:
        ...
        call    0x39994                      ; -> voice_get_driver_obj: driver object for this voice (cmd 0x64)
        add     esp, 4
        ...
        call    0x39280                      ; -> driver_msg_lookup: resolve the 0x67 handler
        mov     ebx, eax
        cmp     ebx, 0                        ; handler present?
        je      dispatch_play                ;   no -> skip setup
        mov     edi, eax
        push    edi
        call    0x39625                      ; -> alloc_seq_slot: build the voice handle (using that handler)
...
dispatch_play:
        ...
        call    0x39747                      ; -> start_seq: post-start hook on the handle
        add     esp, 4
done:

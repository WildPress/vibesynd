; alloc_seq_slot @ 00039625  (176 bytes) -- hand-written assembly (fully commented).
;
; alloc_seq_slot: allocate a sequence slot. Find the first free entry in the per-sequence
...
;
; State table entries: 0 = free, 1 = loaded/paused, 2 = running (see start_seq/9c).
...
;           0x397f1 set_seq_period (arm seq)   0x39747 start_seq (start seq)
;
alloc_seq_slot:
        push    ebp                              ; 55
        mov     ebp, esp                          ; 8bec
...
; --- first activation: start the timer subsystem ---
        ...
        call    0x397f1                           ; e854010000 -> set_seq_period(16, 0xd68d): arm master
        add     esp, 8                            ; 83c408
        push    0x10                              ; 6a10        master slot = 16
        call    0x39747                           ; e8a0000000 -> start_seq(16): start master (1->2)
        add     esp, 4                            ; 83c404

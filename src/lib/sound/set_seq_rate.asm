; set_seq_rate @ 00039879  (69 bytes) -- hand-written assembly (fully commented).
;
; set_seq_rate: compute a sequence period from a rate argument and arm the slot with it
; (via set_seq_period). If the rate is 0 the period is set to the 0xd68d us sentinel (the
...
; scale (10000/8380 = 1.1933) that program_pit_period_us applies when the PIT is programmed, so
...
; Calls:  0x397f1 set_seq_period(slot, period_us)
;
set_seq_rate:
        push    ebp                              ; 55
        mov     ebp, esp                          ; 8bec
...
; .arm (0x398a1):
        ...
        call    0x397f1                            ; e847ffffff -> set_seq_period(slot, period)
        add     esp, 8                             ; 83c408

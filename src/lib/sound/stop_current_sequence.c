/* stop_current_sequence (stack/cdecl, -4s). Globals-only guard + dispatch.
 * disasm: if (g_music_enabled && g_10b4d && (signed char)g_cur_seq >= 0)
 *             snd_cmd_ab(g_seq_ctx, g_seq_state[g_cur_seq]); g_cur_seq = -1;
 */
...
extern void          snd_cmd_ab(int a, int b);

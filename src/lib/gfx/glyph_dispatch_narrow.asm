; glyph_dispatch_narrow @ 0004cd19  (37 bytes) -- hand-written assembly (fully commented).
;
; glyph_dispatch_narrow: the narrow-glyph counterpart of glyph_dispatch_wide. Reads the
; render-mode flags at 0x105 and:
;   bit 1 -> planar 16-colour offscreen buffer: call glyph_blit_narrow (glyph_blit_narrow)
;   bit 2 -> nothing to draw, just return
;   bit 0 -> just return
; Glyph registers (ax=x, bx=y, cx=height, dx=colour, edi=glyph data) pass through.
;
; Globals/ports: 0x105 render-mode flags. The build uses glyph_dispatch_narrow.c
; (db-transcription); this is the readable companion.
;
glyph_dispatch_narrow:
        test    byte ptr [0x105], 2              ; f6050501000002  -- planar mode?
        je      0x4cd29                          ; 7407       -- no -> try next mode (-> try_bit2)
        call    0x4cd3e                          ; e817000000  -- glyph_blit_narrow: draw narrow glyph (planar)

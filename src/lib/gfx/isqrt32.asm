; isqrt32 @ 0004d393  (54 bytes) -- hand-written assembly (fully commented).
;
...
; noted in manifest/names.json for isqrt16), i.e. it computes 2-D vector length.
; The 16-bit sibling is isqrt16.
;
; Arg (stack / cdecl):  [ebp+8] = n        Returns: eax = floor(sqrt(n))
; Globals: 0x3fc81 = per-bit-length initial-guess seed table (words).
; The build uses isqrt32.c (db-transcription); this is the readable companion.
;
isqrt32:
        push    ebp                              ; 55
        mov     ebp, esp                         ; 8bec

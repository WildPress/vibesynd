; draw_tile_region @ 00040f4e  (301 bytes) -- hand-written assembly (fully commented).
;
...
; each one through the tile router iso_tile_dispatch. It appears to draw one diagonal run
...
; (off the planar buffer). Each drawn column calls iso_tile_dispatch with, on the stack:
...
; Reads:   0x105 render flags   0xa7dc per-column byte table   calls iso_tile_dispatch
;
; The build uses draw_tile_region.c (a db-transcription of the raw bytes); this .asm is
...
column_loop:
        ...
        push    edi                              ; --- args for iso_tile_dispatch / its handler ---
        ...
        call    0x4107b                          ; -> iso_tile_dispatch (route to quadrant draw)

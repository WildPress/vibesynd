/* frameless @ 0x15e38: masked blit of a param_5 x param_6 cell into the g_5368 buffer.
   mask = param_1*param_6*param_5 + param_7; fill = g_b46a[param_2]; dest origin =
   g_5368 + param_3 + param_4*0x140. For each of param_6 rows x param_5 cols: write 0
   ...
   the g_5368 load, and the fill deref in an order no source spelling reproduces (we load
                  ...
                  unsigned short param_4, unsigned char param_5, unsigned char param_6, int param_7)
{
    ...
    unsigned char *dest = g_5368 + param_3 + param_4 * 0x140;

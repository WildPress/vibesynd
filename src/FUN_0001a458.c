/* FUN_0001a458 @ 0x1a458 (jump-table dispatcher, 45-entry table at manifest
 * 0x1a39c = jmp CS:[edx*4+0xcc54]; table precedes the fn, 8-byte gap).
 * TRUE SIZE 845 (0x34d): 0x1a458..0x1a7a4 incl.
 *
...
void FUN_0001a458(unsigned char *b)
{
    ...
    case 0x29:
    {
        ...
        if (anim != 1) {
            ...
            frame = *(unsigned short *)(b + 0xe)
                  + *(unsigned short *)(dtype * 2 + (char *)g_a546)
                  + ((b[0x1a] + 0x10) / 32 & 7);
        } else {
            unsigned char *node = g_810e + *(unsigned short *)(b + 0x44);
            if (node >= &g_11670)
    ...
    case 0x18:
    {
        ...
        if (a != 1)
            ...
            frame = *(unsigned short *)(b + 0xe) + 0xcb
                  + ((b[0x1a] + 0x20) / 64 & 1) * 2;
        break;
    }
    case 0x19:

/* MATCHED @ 0x26e18 (130/130 reloc-aware, -4s -oneatx -zp8 -s -zq).
   ...
   levers: (1) pointer statement `q = g_810e + head;` (proven on sibling 0x26da8)
   gives the unfolded `add ebx,0x810e` + disp8 store instead of the symbol folded
   into a disp32; (2) STATEMENT ORDER: computing `cell = &g_10e[idx]` BEFORE
   `pid = p - g_810e` lets idx die straight into the address calc, so it stays in

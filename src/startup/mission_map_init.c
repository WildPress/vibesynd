/* BEHAVIOURALLY EQUIVALENT (verified 2026-07-21): scheduler-fill tie. Sole residual is
   ...
   Independent register-zero vs independent store reordered; identical writes (g_rec6_cursor=g_rec6_table+6,
   g_target_pending=g_10b3e=g_timer_tick=0), same calls. Rest of the 415B body is byte-identical.
   ...
   fmt g_204 with the level byte g_4170[g_cur_mission]) and loads it (FUN_188e8 into
   g_5378), clears + presents the screen buffer g_screen_buf (0x4993b/0x4987e), sets
   g_rec6_cursor = g_rec6_table+6, zeroes three state bytes, inits the 3 pools (FUN_22768) and
   ...
   (1) STORE-SINK fixed: marking g_rec6_cursor `volatile` pins `mov [g_rec6_cursor],eax` right after
...
extern volatile int g_rec6_cursor;
extern unsigned char g_target_pending, g_10b3e, g_timer_tick;
...
void mission_map_init(void)
{
    ...
    if (g_radar_detail != 0)
        ...
        sprintf(buf, g_204, g_4170[g_cur_mission]);

...
def symbolize(name, addr, size):
    ...
    lines = []
    lines.append("    .text")
    lines.append("    .globl %s%s" % (US, name))
    lines.append("%s%s:" % (US, name))
    i = 0
    ...
    while i < size:
        ...
        if o in reloc:
            ...
            if kind == "data":
                objn, toff, add = val
                expr = "%s__obj%d + 0x%x" % (US, objn, toff)
                if add:
                    expr += " + 0x%x" % add
                lines.append("    .long %s" % expr)
            else:
                sym = names.get(val, "fn_%x" % val)
                lines.append("    .long %s%s - . - 4" % (US, sym))
            i += 4
        else:
...
if __name__ == "__main__":
    args = sys.argv[1:]
    if "--underscore" in args:                 # Windows COFF leading-underscore convention
        US = "_"; args = [a for a in args if a != "--underscore"]

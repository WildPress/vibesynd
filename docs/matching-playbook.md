...
- **The disassembly is authoritative.** The Ghidra *decompile* is a hint that regularly invents
  ...
  artefacts. Always confirm against `disassemble_function`.
...
- **Compiler/RTL identity is settled: Watcom C/C++ 9.5, small-model `CLIB3S` (stack-calling).**
  ...
  register-role walls are NOT a wrong-version artefact, they're genuine 9.5 source/flag sensitivity.
...
| `-3s -d2 -oneatx -zp8 -s -zq` | **LEAF runtime-library fns** (`0x3a000+`) that have a full `55 89 e5 … 5d c3` ebp frame yet call nothing. `-of` only frames *callers*, so leaves stay frameless (`8b 44 24 04`) and never match. `-d2` (full debug) FORCES the ebp frame on leaves, `-oneatx` keeps the body optimised so only the frame is added. The CLIB3S runtime was built `-d2`-optimised. Unlocked the C-runtime leaf class: labs 0x3aed8, toupper 0x3dce5, tolower 0x3da37, strchr 0x3e7f7, strcpy-variant 0x3dfcf, path-sep helper 0x3e361, nibble→hex 0x3b9ee. |
...
| `-3s -d2 …` also fixes **push-ebx-BEFORE-frame** non-leaves (`53 55 89 e5` prologue, ebx saved outside the ebp frame): `-of`/`-4s` emit `55 89 e5 … 53` (frame first). NOTE with this prologue the params shift, arg1 is at `[ebp+0xc]`, arg2 at `[ebp+0x10]` (the saved ebx sits at `[ebp+4]`), so a 1-arg fn reads `[ebp+0xc]`. BLOCKED past the prologue by the **shared-epilogue tie**: the CLIB build routes every `return` through one `pop;pop;ret` via `jmp`, but our Watcom DUPLICATES the epilogue per return path (single-return-var C form regresses to a spilled `esi` instead). So multi-return framed-ebx CLIB fns (getc 0x3d3e4, list-search 0x3b99e) stay db, single-return ones may match with `-3s -d2`. |
...
- **HAND-ASM CLIB functions: replicate the `#pragma aux` verbatim.** Some CLIB funcs are inline
  ...
  "db 0" "db 0"`). This banked strcpy 0x3a8d7 byte-exact and generalises to outp/memcpy/etc.
...
- **Negative guards preserve cmp orientation (cont. 21)**. An `&&` chain makes Watcom
  canonicalise `A >= B` into a reversed `cmp B,A / jg`. Writing `if (A < B) goto skip;` keeps the
...
## 3. Walls (recognise, then PARK, not source-reachable)
...
- **Scaled-index lea materialisation (cont. 21)**. Target materialises `2*minor` as the 7-byte
...
- **Cluster-wide dead callee-save**. Some 0x39xxx fns wrap a frameless body in a lone dead
  ...
  `xor eax,eax`. The 0x39xxx region shares the RTL different-Watcom flavour for some fns. (0x39188.)
- **Prologue reg-save-order (framed CLIB region 0x3a000+)**. A framed fn that ALSO saves a callee
  ...
  `[EBP+0xc+]`). Watcom 9.5b at every optimised recipe (`-3s -of -oneatx` and all `-o*` variants,
  `-3`/`-4`) emits frame-FIRST `push ebp; mov ebp,esp; push ebx`. Only `-od` gives regs-first but
  de-optimises the body. Diverges at byte 0. Recognise: the target has `push <reg>` before
  ...
  **ALSO IN THE GAME REGION (cont. 25):** a call-heavy game fn whose params must stay
  ...
  de-optimises the body, same mutual-exclusion as the CLIB region. Recognise: target has

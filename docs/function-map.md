...
Semantic names and descriptions over the `FUN_<addr>` anchors (the source filename stays `FUN_<addr>`, this is the readable layer). Full machine-readable map in `manifest/names.json`.
...
| `0x3a900` | ***(unnamed)*** | lib | GAME hand-asm (NOT CLIB, 0% RTL fingerprint). Word-wise string append @ 0x3a900. Custom frame (PUSH ESI/EDI/EB |
...
| `0x3ab69` | ***(unnamed)*** | lib | GAME hand-asm (NOT CLIB, 0% RTL fingerprint). Thin wrapper @ 0x3ab69 that forwards its 3rd stack arg ([EBP+0xc |
| `0x3adb2` | ***(unnamed)*** |  |  |

...
porting, or to learn how a compiler thinks. Do **not** use it if you only need the game to
...
Hand-iteration finds most matches. A **permuter** closes the near-misses and *proves*
...
- **Declaration-order permutation is the highest-value lever.** Compilers assign stack slots (and
  ...
  for ≤7 locals and sampled beyond, settles most spill-slot/offset near-misses. Watcom lays locals
...
original translation unit's header ABI, and only adds `ebx` to the clobber set without changing

...
The remaining route is not the C. It is a tool that permutes the generated machine code within the space of provably-equivalent encodings and keeps the one that matches, working at the level where the tie actually lives. That is a different project from writing better C, and it is the only lever the evidence leaves standing.
